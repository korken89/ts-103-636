...
mod tests {
    ...
    fn golden_vector_minimal() {
        ...
        let parts = NetworkBeaconParts {
            ...
            next_cluster_channel: AbsoluteChannel::try_from_u16(0x0555).unwrap(),
    ...
    fn golden_vector_full() {
        ...
        let parts = NetworkBeaconParts {
            ...
            next_cluster_channel: AbsoluteChannel::try_from_u16(0x1ABC).unwrap(),
            time_to_next: 0xDEAD_BEEF,
            cluster_max_tx_power: Some(TransmitPower::Dbm13), // code 0b1011 = 11
            current_cluster_channel: Some(AbsoluteChannel::try_from_u16(0x0123).unwrap()),
            additional_channels: Vec::from_slice(&[
                AbsoluteChannel::try_from_u16(0x0234).unwrap(),
                AbsoluteChannel::try_from_u16(0x0345).unwrap(),
                AbsoluteChannel::try_from_u16(0x0456).unwrap(),
            ])
            .unwrap(),
        };
        let mut buf = [0u8; 32];
    ...
    fn network_beacon_parts_round_trip_minimal() {
        let parts = NetworkBeaconParts {
            ...
            next_cluster_channel: AbsoluteChannel::try_from_u16(0x0100).unwrap(),
    ...
    fn network_beacon_parts_round_trip_full() {
        let additional_channels = Vec::from_slice(&[
            ...
            AbsoluteChannel::try_from_u16(0x0400).unwrap(),
        ])
        .unwrap();
        let parts = NetworkBeaconParts {
            power_const: PowerConst::Constrained,
            ...
            current_cluster_channel: Some(AbsoluteChannel::try_from_u16(0x00FE).unwrap()),
            additional_channels,
        };
        let mut buf = [0; 32];
        let n = parts.serialize(&mut buf).unwrap();
